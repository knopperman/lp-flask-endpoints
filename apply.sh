...
original_author="knopperman"
original_name="lp_flask_endpoints"
original_urlname="lp-flask-endpoints"
original_description="Awesome lp_flask_endpoints created by knopperman" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/lp_flask_endpoints "${name}"
...
then
    ...
    echo "then run 'lp_flask_endpoints' or 'python -m lp_flask_endpoints'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'lp_flask_endpoints' or 'python -m lp_flask_endpoints' or 'poetry run lp_flask_endpoints'"
fi
